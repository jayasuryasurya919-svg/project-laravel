    ...
    <div class="rk-bg">
        <div class="rk-wrap">
            <div class="rk-card">
                ...
                <div class="rk-title">Create Account</div>

                <form method="POST" action="{{ route('register', absolute: false) }}" class="space-y-4">
                    @csrf
                    <div>
                        <x-input-label for="name" :value="__('Nama')" class="text-slate-200" />
                        <x-text-input id="name" class="rk-input mt-1" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="email" :value="__('Email')" class="text-slate-200" />
                        <x-text-input id="email" class="rk-input mt-1" type="email" name="email" :value="old('email')" required autocomplete="username" />
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="password" :value="__('Password')" class="text-slate-200" />
                        <x-text-input id="password" class="rk-input mt-1" type="password" name="password" required autocomplete="new-password" />
                        <x-input-error :messages="$errors->get('password')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="password_confirmation" :value="__('Konfirmasi Password')" class="text-slate-200" />
                        <x-text-input id="password_confirmation" class="rk-input mt-1" type="password" name="password_confirmation" required autocomplete="new-password" />
                        <x-input-error :messages="$errors->get('password_confirmation')" class="mt-2" />
                    </div>
                    <button class="rk-btn" type="submit">Register</button>
                </form>

                <div class="mt-4">
                    <a class="rk-google" href="{{ route('google.login') }}">Register dengan Google</a>
                </div>
            </div>
            ...
            <div class="rk-hero">
                <div class="rk-pill mb-6">
                    ...
                    <a class="active" href="{{ route('register') }}">signup</a>
                </div>
